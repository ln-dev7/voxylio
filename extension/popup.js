...
(() => {
  ...
  function renderQuota(ent) {
    ...
    for (const [valId, fillId, barId, remaining, total, unit] of rows) {
      ...
      $(valId).textContent = unit === "minutes" ? fmtInt(Math.floor(rem / 60)) + " min" : fmtCompact(rem);
      const fill = $(fillId);
      fill.style.width = pct + "%";
      fill.classList.toggle("low", pct > 0 && pct <= 20);
      fill.classList.toggle("out", pct === 0);
      $(barId).title = unit === "minutes" ? Math.floor(rem / 60) + " / " + Math.floor(tot / 60) + " min" : fmtInt(rem) + " / " + fmtInt(tot);
    }
