...
(() => {
  ...
  function render(settings2) {
    ...
    $("proVoice").checked = !!settings2.proVoice;
    $("proAudio").checked = !!settings2.proAudio;
    ...
    switch (resp.state) {
      case "ready": {
        ...
        if (resp.ytDubbedDefault) {
          add(
            line(
              t("statusYtDubbed") || "YouTube double d\xE9j\xE0 cette vid\xE9o dans ta langue (piste audio automatique) \u2014 tu entends deux voix. Dans le lecteur : \u2699\uFE0F \u2192 Piste audio \u2192 choisis la version originale, puis recharge la page.",
              "warn"
            )
          );
        }
      ...
      case "subs-loading":
        ...
        break;
      case "no-subs":
        add(line(t("statusNoSubs") || "Aucun sous-titre d\xE9tect\xE9 \u2014 assure-toi que les sous-titres (CC) sont activ\xE9s dans le lecteur. S'il n'en propose pas, le doublage n'est pas possible sur cette vid\xE9o.", "warn"));
        break;
      case "enable-subs":
      ...
      case "pro-site":
        add(
          line(
            "\u2605 " + (t("statusProSite") || "Ce site est r\xE9serv\xE9 au Pro. En gratuit : YouTube, Netflix, Prime Video, Disney+ et Twitch."),
            "warn"
          )
        );
        break;
      case "audio-live":
        add(
          line(
            "\u{1F399} " + (t("statusAudioLive") || "Aucun sous-titre : transcription en direct de l'audio \u2014 le doublage suit dans quelques secondes."),
            "ok"
          )
        );
        break;
      case "audio-quota":
        add(
          line(
            t("statusAudioQuota") || "Minutes Premium Audio \xE9puis\xE9es pour ce mois \u2014 le doublage sans sous-titres reprend au prochain cycle.",
            "warn"
          )
        );
        break;
      case "audio-unavailable":
        add(
          line(
            t("statusAudioUnavailable") || "Impossible de capturer l'audio de ce lecteur (protection du site) \u2014 le doublage sans sous-titres ne peut pas fonctionner ici.",
            "warn"
          )
        );
        break;
    }
    if (resp.plan !== "pro" && resp.trialDaysLeft != null && !resp.siteFree && resp.state !== "pro-site") {
      add(
        line(
          "\u23F3 " + (t("statusTrialNote", [String(resp.trialDaysLeft)]) || `Essai complet : ce site reste d\xE9bloqu\xE9 encore ${resp.trialDaysLeft} j`)
        )
      );
    }
    return frag;
  ...
  function renderQuota(ent) {
    ...
    for (const [valId, fillId, barId, remaining, total, unit] of rows) {
      ...
      $(valId).textContent = unit === "minutes" ? fmtInt(Math.floor(used / 60)) + " / " + Math.floor(tot / 60) + " min" : fmtCompact(used) + " / " + fmtCompact(tot);
      const fill = $(fillId);
      fill.style.width = Math.min(100, pct) + "%";
      fill.classList.toggle("low", pct >= 80 && pct < 100);
      fill.classList.toggle("out", pct >= 100);
      $(barId).title = unit === "minutes" ? Math.floor(used / 60) + " / " + Math.floor(tot / 60) + " min" : fmtInt(used) + " / " + fmtInt(tot);
    }
  ...
  async function refreshAccount() {
    ...
    try {
      ...
      if (!linked) {
        ...
        btn.classList.remove("ghost");
        note.textContent = t("accountNoteFree") || "D\xE9bloquez la traduction contextuelle et les voix neurales naturelles.";
      }
    } catch (e) {
      setSignedOut(true);
      signedIn = false;
