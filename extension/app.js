...
(() => {
  ...
  function fmtSrtTime(sec) {
    const total = Math.round(Math.max(0, Number(sec) || 0) * 1e3);
    const h = String(Math.floor(total / 36e5)).padStart(2, "0");
    const m = String(Math.floor(total % 36e5 / 6e4)).padStart(2, "0");
    const s = String(Math.floor(total % 6e4 / 1e3)).padStart(2, "0");
    const ms = String(total % 1e3).padStart(3, "0");
    return `${h}:${m}:${s},${ms}`;
  }
...
`);
  ...
  var accountUrl = (hash = "") => {
    ...
    return `https://voxylio.lndev.me/${lang}/account?from=extension` + (IS_SAFARI ? "&ctx=safari" : "") + hash;
  };
  var openAccount = () => {
      ...
      upd.addEventListener("click", async () => {
        ...
        try {
          ...
          if (status === "update_available") {
            ...
            setTimeout(() => {
              if (reloaded) return;
              upd.textContent = t("updNone") || "\xC0 jour \u2713";
              setTimeout(() => {
